# docs


## 目次

### 2月

* 📅 [確定申告の実践](https://github.com/muzudho/blue-return-practice)


### 11月

* 📆 [UEC杯コンピュータ囲碁大会について](./contents/UEC杯コンピュータ囲碁大会について/README.md)


### 不定期

* 📁 C#
    * [コメント・テンプレート](./contents/c-sharp/comment-template/README.md)
* 📁 [デスクトップアプリ　＞　Tauri](./contents/desktop-tauri/README.md)
* Visual Studio Code
    * [ターミナルのデフォルトをPowerShellからCommandPromptへ変更](./contents/visual-studio-code/settings.md#ターミナルのデフォルトをPowerShellからCommandPromptへ変更)
    * [ターミナルの文字エンコーディングをUTF-8に変更](./contents/visual-studio-code/settings.md#ターミナルの文字エンコーディングをUTF-8に変更)
    * [Go言語のツールのの文字エンコーディングをUTF-8に変更](./contents/visual-studio-code/settings.md#Go言語のツールのの文字エンコーディングをUTF-8に変更)
* Windows
    * [システムロケールの変更](./contents/windows/settings.md)
* 📖 [デジタル整理術](./contents/デジタル整理術/README.md)
* 📄 [よく使う絵文字](./contents/よく使う絵文字/README.md)
